...
class SubtraktionViewController: UIViewController ,UIAlertViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        highScoreSubtraktion = prefs.integer(forKey: "SavedHighScoreSubtraktion")
    ...
    @IBAction func buttonStartPressed(_ sender: UIButton) {
        ...
        if timerRunning == false {
            timer = Timer.scheduledTimer(timeInterval: 1.0,
                                                           target: self,
                                                           selector: #selector(SubtraktionViewController.counting),
                                                           userInfo: nil,
                                                           repeats: true)
            timerRunning = true
        }
    ...
    func score() {
        ...
        if points > highScoreSubtraktion {
            ...
            prefs.set(highScoreSubtraktion, forKey: "SavedHighScoreSubtraktion")
            highScoreLabel.text = String("Highscore: \(highScoreSubtraktion)")
            prefs.synchronize()
            print("Syncronisiere")
